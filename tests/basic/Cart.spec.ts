...
test("Item is added to the shopping cart", async ({ page }) => {
  ...
  const firstProductName = await firstProductWrapper
    .getByRole("heading")
    .first()
    .textContent();
  const firstProductPrice = await firstProductWrapper
    .locator(".font-bold")
    .textContent();
  ...
  const expectedSubtotal = Number(subtotal?.substring(1)); //substring is for skipping the $ (1) in the price
